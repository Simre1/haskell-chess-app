...
gameFromForest :: [(ByteString, Text)] -> Forest (Ply, Maybe Text) -> Outcome -> Game
...
pureWhiteSpace :: Parser ()
pureWhiteSpace = L.space
  space1 space1 space1

lexeme :: Parser a -> Parser a
lexeme = L.lexeme spaceConsumer

lexemeWhiteSpace :: Parser a -> Parser a
lexemeWhiteSpace = lexemeWhiteSpace
...
movetext pos = (,[]) <$> eog <|> main pos where
  ...
  ply p = do
    ...
    m <- lexemeWhiteSpace $ relaxedSAN p
    snags <- lexemeWhiteSpace $ many nag
    comm1 <- lexeme $ parseComment
    rav <- concat <$> many (lparenP *> var p)
    pure $ (m, \xs -> Node (PlyData pnags m snags (comm1)) xs:rav)
  validateMoveNumber p =
    optional (lexeme $ L.decimal <* space <* many (single periodChar)) >>= \case
      ...
      _ -> pure ()
  parseComment = (Just . T.pack <$> comment) <|> pure Nothing
